...
namespace Kiosk.Device.IT.NV
{
    public partial class NV_Manager
    {
        ...
        private string _deviceName = "NV200 Smart Payout";

        #endregion

        #region Ctor

        public NV_Manager(string kioskName)
        {
        ...
        public bool Connect(string portName)
        {
            ...
            {
                Log.Error(err, "KioskName:{@KioskName} Device Name:{@DeviceName} Transaction Number:{@TransactionNumber} Error Message: {@ErrorMessage}", _kioskName, _deviceName, Guid.NewGuid().ToString(), err.Message);
                return false;
            }
        }
            ...
            for (int i = 1; i < response.DataLen; i++)
            {
                switch (response.Data[i])
                {
                    case POLL_SLAVE_RESET:
                        ...
                        Debug.WriteLine("POLL_SLAVE_RESET");
                        break;
                    case POLL_DISABLED:
                        Debug.WriteLine("POLL_DISABLED");
                        break;
                    case POLL_READ_NOTE:

                        Debug.WriteLine("POLL_READ_NOTE");

                        if (response.Data[i + 1] > 0)
                        {
                            Debug.WriteLine("NOTE_IN_ESCROW");
                    ...
                    case POLL_CREDIT_NOTE:
                        ...
                        Debug.WriteLine("POLL_CREDIT_NOTE_VALUE:" + channelData.Value / 100);
                        break;
                    case POLL_NOTE_REJECTING:
                        Debug.WriteLine("POLL_NOTE_REJECTING");
                        break;
                    case POLL_NOTE_REJECTED:
                        //QueryRejection(log);
                        Debug.WriteLine("POLL_NOTE_REJECTED");
                        break;
                    case POLL_NOTE_STACKING:
                        Debug.WriteLine("POLL_NOTE_STACKING");
                        break;
                    case POLL_FLOATING:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_FLOATING");
                        break;
                    case POLL_NOTE_STACKED:
                        ...
                        Debug.WriteLine("POLL_NOTE_STACKED");
                        break;
                    case POLL_FLOATED:
                    ...
                    case POLL_NOTE_STORED_IN_PAYOUT:
                        ...
                        Debug.WriteLine("POLL_NOTE_STORED_IN_PAYOUT");

                        break;
                    case POLL_SAFE_NOTE_JAM:
                        Debug.WriteLine("POLL_SAFE_NOTE_JAM");
                        break;
                    case POLL_UNSAFE_NOTE_JAM:
                        Debug.WriteLine("POLL_UNSAFE_NOTE_JAM");
                        break;
                    case POLL_ERROR_DURING_PAYOUT:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_ERROR_DURING_PAYOUT");
                        break;
                    case POLL_FRAUD_ATTEMPT:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_FRAUD_ATTEMPT");
                        break;
                    case POLL_STACKER_FULL:
                        Debug.WriteLine("POLL_STACKER_FULL");
                        break;
                    case POLL_NOTE_CLEARED_FROM_FRONT:
                        Debug.WriteLine("POLL_NOTE_CLEARED_FROM_FRONT");

                        i++;
                        break;
                    case POLL_NOTE_CLEARED_TO_CASHBOX:
                        Debug.WriteLine("POLL_NOTE_CLEARED_TO_CASHBOX");

                        i++;
                        break;
                    case POLL_NOTE_PAID_INTO_STORE_AT_POWER_UP:
                        Debug.WriteLine("POLL_NOTE_PAID_INTO_STORE_AT_POWER_UP");

                        i += 7;

                        break;
                    case POLL_NOTE_PAID_INTO_STACKER_AT_POWER_UP:
                        Debug.WriteLine("POLL_NOTE_PAID_INTO_STACKER_AT_POWER_UP");

                        i += 7;

                        break;
                    case POLL_CASHBOX_REMOVED:
                        Debug.WriteLine("POLL_CASHBOX_REMOVED");
                        break;
                    case POLL_CASHBOX_REPLACED:
                        Debug.WriteLine("POLL_CASHBOX_REPLACED");
                        break;
                    case POLL_DISPENSING:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_DISPENSING");
                        break;
                    case POLL_DISPENSED:
                        ...
                        Debug.WriteLine("POLL_DISPENSED");
                        break;
                    case POLL_EMPTYING:
                    ...
                    case POLL_EMPTIED:
                        ...
                        Debug.WriteLine("POLL_EMPTIED");
                        break;
                    case POLL_SMART_EMPTYING:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_SMART_EMPTYING");
                        break;
                    case POLL_SMART_EMPTIED:
                        ...
                        Debug.WriteLine("POLL_SMART_EMPTIED");
                        break;
                    case POLL_JAMMED:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_JAMMED");
                        break;
                    case POLL_HALTED:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_HALTED");
                        break;
                    case POLL_INCOMPLETE_PAYOUT:

                        i += (byte)((response.Data[i + 1] * 11) + 1);

                        Debug.WriteLine("POLL_INCOMPLETE_PAYOUT");
                        break;
                    case POLL_INCOMPLETE_FLOAT:

                        i += (byte)((response.Data[i + 1] * 11) + 1);

                        Debug.WriteLine("POLL_INCOMPLETE_FLOAT");
                        break;
                    case POLL_NOTE_TRANSFERED_TO_STACKER:

                        i += 7;

                        Debug.WriteLine("POLL_NOTE_TRANSFERED_TO_STACKER");
                        break;
                    case POLL_NOTE_HELD_IN_BEZEL:

                        i += 7;

                        Debug.WriteLine("POLL_NOTE_HELD_IN_BEZEL");
                        break;
                    case POLL_PAYOUT_OUT_OF_SERVICE:
                        Debug.WriteLine("POLL_PAYOUT_OUT_OF_SERVICE");
                        break;
                    case POLL_TIME_OUT:

                        i += (byte)((response.Data[i + 1] * 7) + 1);

                        Debug.WriteLine("POLL_TIME_OUT");
                        break;
                    default:
                        break;
                }
            ...
            try
            {
                ...
                if (response.Data[0] == RESPONSE_OK)
                {
                    ...
                    if (DebugMode)
                    {
                        ...
                        //Debug.WriteLine($"result: success, command: {NV_Commands.Command[command]}, count: {_count}");

                        //if (_keys.EncryptKey != null)
                        //{
                        //    Debug.WriteLine($"decrypt key: {BitConverter.ToString(_keys.EncryptKey)}");

                        //}

                        //Debug.WriteLine($"COM-> {response.RequestDataHex}");
                        //Debug.WriteLine($"COM<- {response.ResponseDataHex}");
                        //Debug.WriteLine("------------------------------------------------------------");
                ...
                else
                {
                    ...
                    if (DebugMode)
                    {
                        ...
                        //Debug.WriteLine($"COM-> {response.RequestDataHex}");
                        //Debug.WriteLine($"COM<- {response.ResponseDataHex}");
                        //Debug.WriteLine($"result: fail, command: {NV_Commands.Command[command]}, count: {_count}");
                        //Debug.WriteLine($"error messsage: {response.ErrorMessage}, error code: {response.ErrorCode}");
